...
static int my_probe(struct platform_device *pdev)
{
	...
	pr_info("Name device : %s",pdev->name);
	led = devm_kzalloc(&pdev->dev, sizeof(*led), GFP_KERNEL);
	pr_info("Address led : %p\n",led);
	if (led == NULL) {
	 ...
	 */
	led->led_pin = of_get_named_gpio(np, "my_led", 0);
	...
	ret = gpio_request(led->led_pin, "gpio_54");
	if (ret) {
	...
	__gpio_set_value(led->led_pin, 1);
...
static int my_remove(struct platform_device *pdev)
{
	...
	__gpio_set_value(led->led_pin, 0);
